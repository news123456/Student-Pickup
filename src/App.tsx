...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-background text-text-primary font-sans selection:bg-accent-emerald/30">
      {/* Navigation Bar */}
      <nav className="h-[70px] border-b border-surface-border px-8 flex items-center justify-between sticky top-0 z-50 backdrop-blur-xl bg-background/80">
        <div className="flex items-center space-x-3">
          <div className="w-8 h-8 bg-accent-emerald rounded-lg flex items-center justify-center group shadow-[0_0_15px_rgba(16,185,129,0.3)]">
              ...
              <div className="lg:col-span-8 flex flex-col">
                <div className="glass-card overflow-hidden bg-black flex-1 relative flex flex-col">
                  <div className="absolute top-4 left-4 z-10">
                    <div className="status-badge text-accent-emerald bg-black/40 backdrop-blur-md">
                ...
                <div className="sticky top-28 space-y-6">
                  <AnimatePresence mode="wait">
                    {matchedEntry ? (
                      ...
                      >
                        <div className="p-8 border-b border-surface-border bg-accent-emerald-alpha/5 relative overflow-hidden group">
                           <CheckCircle2 className={cn(
                             "w-32 h-32 absolute -right-4 -bottom-4 rotate-12 transition-all duration-500",
                             matchStatus.guardian && matchStatus.student ? "text-accent-emerald/20 scale-110" : "text-white/5 opacity-40"
                           )} />
                           <div className="relative">
                            <div className={cn(
                               "status-badge mb-4 border-accent-emerald/20",
                            ...
                            <p className="text-[10px] uppercase font-bold tracking-[0.2em] text-accent-emerald mt-2">
          ...
          {activeTab === 'history' && (
            ...
            >
              <div className="glass-card overflow-hidden">
                <div className="p-10 border-b border-surface-border flex items-center justify-between">
                  <div>
                    <div className="status-badge text-accent-emerald bg-accent-emerald-alpha mb-3">Audit Logs</div>
                    <h2 className="text-3xl font-black text-white italic tracking-tighter uppercase mb-1 leading-none">Access History</h2>
                    <p className="text-text-secondary text-xs font-medium tracking-tight">Full historical log of campus student releases.</p>
                  </div>
                  <div className="flex items-center space-x-3">
                    <button 
                        ...
                        {recentPickups.length > 0 ? (
                          ...
                          <tr>
                            <td colSpan={4} className="p-24 text-center">
                              ...
                              <p className="text-text-secondary text-xs uppercase font-bold tracking-widest">Database records empty</p>
                            </td>
                          </tr>
                        )}
                      </tbody>
...
function NavBtn({ active, onClick, label, icon }: { active: boolean; onClick: () => void; label: string; icon: React.ReactNode }) {
  return (
    <button 
      onClick={onClick}
      className={cn(
        "px-6 py-2 rounded-lg transition-all flex items-center space-x-2 group cursor-pointer",
...
function RegisterForm({ onEnroll }: { onEnroll: (entry: RegistryEntry) => void }) {
  ...
  return (
    <div className="glass-card overflow-hidden">
       <div className="p-10 border-b border-surface-border">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-3xl font-black text-white italic tracking-tighter flex items-center space-x-3 uppercase">
            <UserPlus className="w-8 h-8 text-accent-emerald" />
            <span>SECURE BIOMETRIC ENROLLMENT</span>
          </h2>
          <div className="status-badge text-accent-emerald bg-accent-emerald-alpha">Multi-Guardian v3.0</div>
        </div>
        <p className="text-text-secondary text-xs font-medium max-w-lg">Enroll the student and multiple authorized guardians for flexible and secure identity verification.</p>
      </div>
      ...
      <div className="grid md:grid-cols-2">
        ...
        <div className="bg-black/40 p-10 flex flex-col items-center justify-center border-l border-surface-border transition-all">
          {!isCapturing && step !== 'details' ? (
            <button 
              onClick={startCamera}
              className="w-full aspect-[4/5] border-2 border-dashed border-white/10 rounded-3xl hover:border-accent-emerald/40 hover:bg-accent-emerald/5 transition-all group flex flex-col items-center justify-center cursor-pointer"
            >
              <div className="w-20 h-20 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-accent-emerald/20 transition-all">
                <Camera className="w-8 h-8 text-white/20 group-hover:text-accent-emerald" />
              </div>
              <p className="text-[11px] font-black text-text-secondary mt-6 group-hover:text-white uppercase tracking-[0.25em]">ACTIVATE SENSOR</p>
            </button>
          ) : isCapturing ? (
            <div className="relative w-full aspect-[4/5] rounded-[2.5rem] overflow-hidden bg-black shadow-2xl ring-4 ring-white/5 ring-inset ring-offset-8 ring-offset-background group">
              <video ref={videoRef} autoPlay muted className="w-full h-full object-cover" />
              <div className="absolute inset-x-0 top-0 bottom-0 flex items-center justify-center pointer-events-none">
                 <div className="w-48 h-64 border-2 border-accent-emerald/20 rounded-[80px] shadow-[0_0_100px_rgba(16,185,129,0.1)]" />
              </div>
             ...
             <div className="w-full aspect-[4/5] bg-surface/50 rounded-3xl flex items-center justify-center border border-white/5">
  ...
  return (
    ...
    >
      ...
      <div className="glass-card overflow-hidden">
                ...
                <td className="px-6 py-5">
                   <div className="flex -space-x-2">
                      <div className="w-10 h-10 rounded-lg border-2 border-surface bg-white/5 overflow-hidden ring-2 ring-black">
                        {person.studentPhoto ? <img src={person.studentPhoto} alt="Student" /> : null}
                      </div>
                      {person.guardians?.map((g, gi) => (
                        <div key={gi} className="w-10 h-10 rounded-lg border-2 border-surface bg-white/5 overflow-hidden ring-2 ring-black" title={g.name}>
                          {g.photo ? <img src={g.photo} alt={g.role} /> : null}
                        </div>
                      ))}
                   </div>
                </td>
        ...
        {registry.length === 0 && (
          <div className="p-20 text-center text-text-secondary uppercase text-[10px] font-black tracking-widest">
